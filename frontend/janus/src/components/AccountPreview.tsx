...
export interface AccountPreviewProps {
   ...
   * @property {'expanded'} expanded - Displays the user's avatar, username, and address if available.
   * @default 'condensed'
   */
...
function AccountPreview(props: AccountPreviewProps) {
  ...
  const {auth: session} = React.useContext(AuthContext);
  ...
  return (
    <Tooltip title={session.user.username ?? accountLocaleText?.accountPreviewTitle}>
